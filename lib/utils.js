...
module.exports.getModules = function getModules() {
    ...
    // stupid quick bug fix
    var node_modules = null;
    if (path.existsSync(path.join(cwd, './node_modules/'))) {
        node_modules = path.join(cwd, './node_modules/');
    }
    else if (path.existsSync(path.join(__dirname, '../../node_modules/'))) {
        ...
        lines.forEach(function(line, index) {
            ...
            if(frame.filename[0] === '/' || frame.filename[0] === '.') {
                ...
                if(frame.filename in cache) {
                    ...
                    fs.readFile(frame.filename, 'utf8', function(err, file) {
                        if(!err) {
                            if (/\.coffee$/.test(frame.filename)) file = coffee.compile(file).toString().split('\n');
                            else file = file.toString().split('\n');
